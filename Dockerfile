...
# Always bind the HTTP API on all container interfaces so Docker port mapping works,
# even when users pass their own relay flags and replace CMD at `docker run` time.
ENTRYPOINT ["python3", "/app/acp_relay.py", "--http-host", "0.0.0.0"]
