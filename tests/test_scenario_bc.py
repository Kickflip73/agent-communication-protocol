...
Run: python3 tests/test_scenario_bc.py
     pytest tests/test_scenario_bc.py  (also works)
"""
...
def run_bc_tests():
    """Run all Scenario B+C tests. Returns (passed, total, failed_list)."""
    global results, _procs
    results = []
    _procs = []
    ...
    failed_list = [(name, note) for name, p, note in results if not p]

    print(f"\n{'='*55}")
    print(f"Scenario B+C Tests: {passed}/{total} PASS")
    if failed_list:
        print(f"\nFailed:")
        for name, note in failed_list:
            print(f"  ❌ {name}" + (f" — {note}" if note else ""))
    print("="*55)

    return passed, total, [name for name, _ in failed_list]
