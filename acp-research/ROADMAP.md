...
### ✅ 已实现（轻量子集）— 原生 Pub/Sub（A2A PR#1196）
- **2026-04-12 更新**：ACP v3.9.0 实现了 topic-based Pub/Sub 轻量子集（4 个端点），是 A2A #1196 的**首个工作参考实现**
- 设计取舍：仅内存订阅（重启清空），无消息持久化（离线场景用 `--persist-queue`），无 dead-letter queue
- A2A 上游仍处于 proposal 阶段（3 comments，无实现），ACP 率先落地
