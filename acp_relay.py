...
v0.6 changes (2026-03-20):
  - Multi-session peer registry: concurrent connections to N peers simultaneously
  - /peers endpoint: list all connected peers with stats
  - /peer/{id}/send endpoint: send message to a specific peer by session_id
  - /peer/{id} endpoint: get single peer info
  - AgentCard v2: capabilities.multi_session=true, endpoints.peers + peer_send
  - Peer connect/disconnect events include peer_count for observability
...
# _peers: { peer_id -> {ws, name, card, connected_at, link, messages_sent, messages_received} }
_peers: dict = {}
...
# ══════════════════════════════════════════════════════════════════════════════
# Multi-session peer management (v0.6)
# ══════════════════════════════════════════════════════════════════════════════

def _peer_register(peer_id: str, ws, name: str = None, card: dict = None, link: str = None) -> dict:
    """Register or update a peer connection in the global registry."""
    with _peers_lock:
        ...
        _peers[peer_id] = entry
        return entry


def _peer_unregister(peer_id: str):
...
def _peer_list() -> list:
    """Return a serializable list of connected peers (no ws object)."""
    with _peers_lock:
        result = []
        for pid, p in _peers.items():
            result.append({
                ...
                "messages_sent":     p.get("messages_sent", 0),
                "messages_received": p.get("messages_received", 0),
            })
        return result


def _peer_get_ws(peer_id: str):
    """Get ws handle for a specific peer (None if not found)."""
    with _peers_lock:
        p = _peers.get(peer_id)
        return p["ws"] if p else None


def _peer_primary_ws():
    """Return the primary peer ws (first connected / legacy compat)."""
    global _peer_ws
    if _peer_ws:
        return _peer_ws
    # Fallback: return any connected peer's ws
    with _peers_lock:
...
def _peer_increment(peer_id: str, field: str, delta: int = 1):
    """Thread-safe increment of a peer counter."""
    with _peers_lock:
        if peer_id in _peers:
            _peers[peer_id][field] = _peers[peer_id].get(field, 0) + delta
...
def _make_agent_card(name, skills):
    return {
        ...
        "capabilities": {
            ...
            "query_skill":        True,
            "server_seq":         True,
            "multi_session":      True,   # v0.6: multiple concurrent peer connections
        },
        "auth":      {"schemes": ["none"]},
        "endpoints": {
            "send":         "/message:send",
...
async def host_mode(token, ws_port, http_port):
    ...
    async def on_guest(websocket):
        ...
        # Register in multi-session peer registry (v0.6)
        peer_link = f"acp://{getattr(websocket, 'remote_address', ('?', 0))[0]}"
        _peer_register(session_id, websocket, link=peer_link)
        _broadcast_sse_event("peer", {"event": "connected", "session_id": session_id,
                                       "peer_count": len(_peers)})

        print(f"\n{'='*55}")
        print(f"ACP P2P v{VERSION} - peer connected [{session_id}] ({len(_peers)} total)")
        ...
        try:
            ...
            log.info(f"Peer disconnected [{session_id}]")
        finally:
            _peer_unregister(session_id)
            # Only clear primary _peer_ws if this was the last peer
            if not _peers:
                _peer_ws = None
                _status["connected"] = False
                _status["peer_card"] = None
            _broadcast_sse_event("peer", {"event": "disconnected", "session_id": session_id,
                                           "peer_count": len(_peers)})
    ...
    while retry < P2P_MAX_RETRIES:
        try:
            ...
            async with await _proxy_ws_connect(uri, ping_interval=20, ping_timeout=10) as ws:
                ...
                # Register in multi-session peer registry (v0.6)
                _peer_register(session_id, ws, link=f"acp://{host}:{ws_port}/{token}")
                _broadcast_sse_event("peer", {"event": "connected", "session_id": session_id,
                                               "peer_count": len(_peers)})

                print(f"\n{'='*55}")
                print(f"ACP P2P v{VERSION} - {'reconnected' if retry else 'connected'} [P2P] [{session_id}]")
            ...
            # Unregister this peer from multi-session registry
            sid = _status.get("session_id")
            if sid:
                _peer_unregister(sid)
            if not _peers:
                _peer_ws = None
                _status["connected"] = False
                _status["peer_card"] = None
            _broadcast_sse_event("peer", {"event": "disconnected",
                                           "session_id": sid, "peer_count": len(_peers)})
...
class LocalHTTP(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # /peers — list all connected peers (v0.6 multi-session)
        elif p == "/peers":
            self._json({"peers": _peer_list(), "count": len(_peers)})

        # /peer/{id} — get single peer info (v0.6, not a /send path)
        elif p.startswith("/peer/") and not p.endswith("/send"):
            peer_id = p[len("/peer/"):]
            with _peers_lock:
                p_entry = _peers.get(peer_id)
            if p_entry:
                safe = {k: v for k, v in p_entry.items() if k != "ws"}
                self._json(safe)
            else:
    ...
    def do_POST(self):
        ...
        # /peer/{id}/send — send message to a specific peer (v0.6 multi-session)
        elif p.startswith("/peer/") and p.endswith("/send"):
            peer_id = p[len("/peer/"):-len("/send")]
            try:
                body = self._read_body()
                ws = _peer_get_ws(peer_id)
                if not ws:
                    self._json({"ok": False, "error": f"peer '{peer_id}' not connected"}, 404)
                    return
                # Build message (same as /message:send but targeted)
                parts = body.get("parts")
                if not parts:
                    text = body.get("text") or body.get("content") or ""
                    parts = [_make_text_part(str(text))] if text else []
                ...
                if len(serialized.encode()) > MAX_MSG_BYTES:
                    self._json({"ok": False, "error": f"message too large (max {MAX_MSG_BYTES} bytes)"}, 413)
                    return
                # Send directly to targeted peer ws
                asyncio.run_coroutine_threadsafe(ws.send(serialized), _loop)
                _peer_increment(peer_id, "messages_sent")
                _status["messages_sent"] = _status.get("messages_sent", 0) + 1
