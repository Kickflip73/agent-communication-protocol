...
> **Reliable Messaging Trio (v3.15–v3.17)** — A2A has no spec-level message delivery guarantees. If you send a message to an offline peer, it's gone. ACP's three-phase rollout covers all patterns: **v3.15 `POST /messages:batch`** provides atomic multi-message enqueue (all-or-nothing, 100 msg limit); **v3.16 Message ACK Protocol** adds per-message delivery confirmation via `require_ack=true` on `POST /message:send` (+ `ack_timeout_ms` param, 5s default, max 30s, `ERR_ACK_TIMEOUT` 408); **v3.17 `POST /messages:stream`** completes the trio with a persistent WebSocket inlet — client sends frames continuously, relay routes each with `{ok,message_id,server_seq}` per-frame ack. Together these form the Reliable Messaging Trio, covering batch, request-reply, and streaming delivery patterns. No A2A equivalent.
...
| Batch messages (v3.15) | _(always on)_ | `POST /messages:batch` — atomic multi-message enqueue, 100 msg limit, per-message results |
| Message ACK (v3.16) | _(always on)_ | `POST /message:send?require_ack=true` — per-message delivery confirmation, `ERR_ACK_TIMEOUT` 408, 30s max TTL |
| Streaming inlet (v3.17) | _(always on)_ | `POST /messages:stream` — persistent WebSocket, per-frame routing + `{ok,message_id,server_seq}` ack |

---

## Reliable Messaging Trio (v3.15–v3.17)

ACP's message delivery guarantees evolved through a three-phase rollout:

| Version | Feature | Guarantee |
|---------|---------|-----------|
| v3.15 | `POST /messages:batch` | Atomic multi-message enqueue (all-or-nothing) |
| v3.16 | Message ACK Protocol | Per-message delivery confirmation (request-reply) |
| v3.17 | `POST /messages:stream` | Persistent WebSocket, per-frame routing with real-time ack |

> Together these form the **Reliable Messaging Trio** — covering batch, request-reply, and streaming patterns. A2A has no equivalent.
